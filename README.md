...
| 模块 | 说明 |
|------|------|
| 语音记账 | 按住说话「王阿姨买了两个肉包一杯豆浆，6块」，AI 自动解析顾客/商品/金额，熟客自动归档。**没听出金额时不落库，当场追问「这笔多少钱」**；记完摆出「我这么记的，对吗？」可当场改金额/收支方向（留痕可查） |
...
python scripts/eval_ai_parse.py        # 用真实模型评测「一句话记账」准确率（耗额度）
python ../scripts/prewarm_cache.py     # 预热 AI 缓存（需先起后端）
```

**`eval_ai_parse.py` 是唯一会验证核心承诺的脚本**：拿 20 条真实店主原话去调真实模型，
逐条核对金额/方向/分类/熟客。其余测试全都是打桩的 —— 也就是说，
"说一句话就能记好账"这件事只有它验过。实测参考值：**金额 20/20、熟客 20/20、
方向 19/20、分类 18/20**。
